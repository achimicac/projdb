...
export const article = (req, res, next) => {
      ...
      try {
            db.query("SELECT DISTINCT Articles.* FROM User INNER JOIN Pet ON User.id = Pet.id INNER JOIN Articles ON Pet.petType = Articles.petType WHERE User.id = ?", [1], (err, result) => {
